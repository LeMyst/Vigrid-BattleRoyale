...
- ⚠️ **At the default weight the derived table does NOT match the authored one, and that is expected.** Every circle is nested around a village seed (`end_in_villages`), so every circle reads denser than the map mean. Set `zone_poi_density_weight` to 0 to check the radius term alone, which is what reproduces `min_players` entry for entry.
