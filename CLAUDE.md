...
- ⚠️ **`VIGRID_AUTORUN_MOVEMENT_ANGLE` is the one number to flip first** if the character veers or refuses to move. It ships `0` — what `DisableInput` passes, and the obvious "no deviation" — but vanilla's own call sites pass `1`, both the AI bot (`bot_hunt.c:145`) and the camera tools (`ctevent.c:50`), with no documented unit anywhere. Neither reading is measured; it is a named constant so the experiment stays one variable.
