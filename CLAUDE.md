...
- Client: `SetHotZones` / `ClearHotZones` — **hot zones ride the same push contract**, from the same call site and the same destructor. They are static, admin-configured circles from `zone_settings.json` (`hot_zone_centers` / `hot_zone_radii`, v5) marking regions of interest, and they are **purely cosmetic** — nothing reads them but the three renderers. Drawn *under* the play-area rings on both the fullscreen map and the minimap, so a hot zone can never obscure the circle a player has to run to; drawn **filled** on the spawn-selection screen, which is a one-off zoomed-out decision rather than something panned around. Two things are load-bearing: they carry their **own** sequence number (`GetHotZoneSeq`, and `BattleRoyaleRPC.hot_zone_seq` on the BR side) because they arrive on a separate RPC that often lands while a map is already open — folded into the edge check, they would otherwise wait out the 1 s watchdog; and delivery is **per-identity from `BattleRoyaleServer.PlayerLoadedIn`**, not a broadcast, which is what covers a mid-match joiner. `BattleRoyaleZoneData.Validate()` truncates the two arrays to equal length and drops anything undrawable, so no consumer re-checks the pair.
