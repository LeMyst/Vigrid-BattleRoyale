...
- `Extra/` holds 17 independent single-purpose sub-addons, each its own PBO. 16 are built; `Extra/DisableFogChernarusPlus/` is parked as `config.cpp.disabled` and produces no PBO. Most are small script tweaks; the exceptions are `Extra/KillFeed/`, `Extra/SafeZone/`, `Extra/Map/` and `Extra/AutoRun/`, self-contained addons documented under *Architecture → Kill feed*, *→ Safe zone / lobby truce*, *→ Map* and *→ Auto-run*. Each folder carries its own `README.md`, indexed by `Extra/README.md`.
