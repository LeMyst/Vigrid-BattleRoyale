/**
 *  Auto-Run - logging. No guard: compiles on both client and server.
 *
 ...
 *    -autorun-trace | -autorun-debug | -autorun-info | -autorun-warn | -autorun-none    (CLI)
 *    AutoRunLogLevel = 1..4 in serverDZ.cfg, negative to silence            (server only)
 *    VIGRID_AUTORUN_LOG_LEVEL                                              (compile default)
 */
class VigridAutoRunLog
{
    ...
    static void LogMessage(int level, string message)
    {
        ...
        if (level == NONE)
        {
            Error2("", stamp + " [AutoRun] " + message);
            return;
        }

        PrintFormat("%1 [AutoRun][%2] %3", stamp, level, message);
    }
    ...
    {
        //--- Command line wins.
        if (IsCLIParam("autorun-none"))
            return false;
        if (IsCLIParam("autorun-trace"))
            return TRACE >= level;
        if (IsCLIParam("autorun-debug"))
            return DEBUG >= level;
        if (IsCLIParam("autorun-info"))
            return INFO >= level;
        if (IsCLIParam("autorun-warn"))
            return WARN >= level;
        ...
        int config_level = GetGame().ServerConfigGetInt("AutoRunLogLevel");
        if (config_level > 0)
        ...
        return VIGRID_AUTORUN_LOG_LEVEL >= level;
    }
}
