/**
 *  Vigrid Map - logging. No guard: compiles on both client and server.
 *
 ...
 *    -map-trace | -map-debug | -map-info | -map-warn | -map-none                 (CLI)
 *    MapLogLevel = 1..4 in serverDZ.cfg, negative to silence           (server only)
 *    VIGRID_MAP_LOG_LEVEL                                              (compile default)
 */
class VigridMapLog
{
    ...
    static void LogMessage(int level, string message)
    {
        ...
        if (level == NONE)
        {
            Error2("", stamp + " [VigridMap] " + message);
            return;
        }

        PrintFormat("%1 [VigridMap][%2] %3", stamp, level, message);
    }
    ...
    {
        //--- Command line wins.
        if (IsCLIParam("map-none"))
            return false;
        if (IsCLIParam("map-trace"))
            return TRACE >= level;
        if (IsCLIParam("map-debug"))
            return DEBUG >= level;
        if (IsCLIParam("map-info"))
            return INFO >= level;
        if (IsCLIParam("map-warn"))
            return WARN >= level;
        ...
        int config_level = GetGame().ServerConfigGetInt("MapLogLevel");
        if (config_level > 0)
        ...
        return VIGRID_MAP_LOG_LEVEL >= level;
    }
}
