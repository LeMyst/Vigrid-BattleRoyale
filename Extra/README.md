...
**19 folders, 17 of which build** — `DisableFogChernarusPlus` and `DumpItemHeights` are currently
parked.
...
Only seven addons declare a `defines[]`, which is what lets the mod guard its call sites with
`#ifdef`:
